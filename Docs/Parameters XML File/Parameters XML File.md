...
|→→→`<startBit>`|Yes|xs:int|1/1|Specifies the Start Bit for the Parameter. It is a 0-based information. For Labels from Rx Channels, range accepted is: [0:31]. For Labels from Tx Channels, valid range is: [8:31] (Label ID (bits [0:7]) are overloaded by board firmware). For Parameters when `<encoding>` is set to BCD, `<startBit>` values supported are: 8..30. Note that the 31st bit of the Tx message can be configured only when the parity attribute is set to "data"; otherwise, it will be overridden by the board firmware.|
