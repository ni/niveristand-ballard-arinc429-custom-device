...
|→→`<transferType>`|No|xs:int|0/1|Specifies the transfer type for a message. Use 0 for a scheduled label; use 1 for an acyclic label. Default Value (if XML element is absent) is 0 (Scheduled).|
